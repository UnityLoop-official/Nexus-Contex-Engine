...
export class ChatPanel {
    ...
    private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, daemonClient: DaemonClient) {
        this.panel = panel;
    ...
    private initializeLLMClient() {
        ...
        if (!apiKey || apiKey.trim().length === 0) {
            vscode.window.showWarningMessage(
                '⚠️ Nexus: OpenAI API key not configured. Please set it in settings.',
                'Open Settings'
            ).then(selection => {
    ...
    public static createOrShow(extensionUri: vscode.Uri, daemonClient: DaemonClient) {
    ...
    public static getHtmlContent(webview: vscode.Webview, extensionUri: vscode.Uri): string {
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
